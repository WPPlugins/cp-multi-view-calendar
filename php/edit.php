...
<?php } ?>
    ...
    <script src="<?php
         if (file_exists(dirname( __FILE__ ).'/../DC_MultiViewCal/language/multiview_lang_'.$this->_autodetect_language().'.js'))
            echo plugins_url('/DC_MultiViewCal/language/multiview_lang_'.$this->_autodetect_language().'.js', dirname(__FILE__) );
        else
        ...
        <form action="<?php echo $this->get_site_url(); ?>/?cpmvc_id=<?php echo $this->calendar; ?>&cpmvc_do_action=mvparse&f=datafeed&calid=<?php echo $_GET["calid"];?>&month_index=<?php echo $_GET["month_index"];?>&method=adddetails<?php echo isset($event)?"&id=".$event["id"]:""; ?>" class="fform" id="fmEdit" method="post">
          ...
          <label>
            ...
            if (isset($dc_locations) && is_array($dc_locations))
            {
                ...
                {
                    echo '<option value="'.rquote_field($dc_locations[$i]).'" '.((isset($event) && ($event["location"] ==$dc_locations[$i]))?"selected":"").'>'.$dc_locations[$i].'</option>';
                }
                echo '</select>';
            }
            else
                echo '<input MaxLength="200" id="Location" name="Location" class="inputtext"  type="text" value="'.((isset($event))?rquote_field($event["location"]):"").'" />';
            ?>
